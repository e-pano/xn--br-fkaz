...
const baseURL = 'https://börü.pages.dev/';

function updateIcon() {
    const iconPath = `${baseURL}icon/`;
    if (isDayTime) {
        soundIcon.src = soundOn ? `${iconPath}togglesoundDay.svg` : `${iconPath}togglemuteDay.svg`;
    } else {
        soundIcon.src = soundOn ? `${iconPath}togglesoundNight.svg` : `${iconPath}togglemuteNight.svg`;
    }
}
...
const images = [
    `${baseURL}img/BGlandscapeDay.jpg`,
    `${baseURL}img/BGlandscapeNight.jpg`,
    `${baseURL}img/BGportraitDay.jpg`,
    `${baseURL}img/BGportraitNight.jpg`,
];
